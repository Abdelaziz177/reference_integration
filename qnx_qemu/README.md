...
- **Optional**: libvirt/virbr0 bridge networking (or use QEMU user-mode networking)
- Valid **QNX SDP 8.0 license** from [my.qnx.com](https://www.qnx.com/)
- Valid **QNX.com credentials** to download the SDP archive
...
#### 2. Clone the toolchain repo

```bash
git clone https://github.com/eclipse-score/toolchains_qnx.git
```

This repo contains the Bazel rules and credential helper used to download and register the QNX SDP toolchain.

#### 3. Provide QNX download credentials
...
#### **4. Download QNX Software Center for Linux**

![image.png](attachment:0aa4cf4b-6e3e-49e8-b9a1-2c4333033998:image.png)

#### **5. Download QNX SDP 8.0 (we need only license files)**

From QNX Software Center, install **QNX Software Development Platform 8.0**.

This contains the toolchains and `license` folder required for activation.

![image.png](attachment:cb3e0a45-ab01-40dc-bcc8-e60baaf5c493:image.png)

#### 6.Source SDP environment:

```bash
source <path-to-toolchains_qnx_sdp>/qnxsdp-env.sh 
echo "$QNX_HOST" "$QNX_TARGET"
```

![image.png](attachment:21b56f64-6839-4c32-81f1-fcfb56b1f0b0:image.png)

#### 7.**Register QNX license**

```bash
qnxlicense -ac /path/to/qnx.lic
qnxlicense -al
```

If Bazel’s sandbox cannot see the license:
...
**Using Bazel launcher**
```bash
bazel run --config=x86_64-qnx //:run_qemu

```
**Additional info**
#### 10. Configure QEMU networking
...
for **user-mode** networking  just add this command in run_qemu.sh
